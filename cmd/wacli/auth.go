...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			res, authed, err := runAuth(flags, opts)
			if err != nil {
				return err
			}
...
// runAuth performs QR/phone-pair authentication and bootstrap sync. It
// returns the sync result plus whether the session is actually authenticated
// afterward: Sync returns nil both on Ctrl-C and after server-side logouts,
// so the caller cannot infer success from a nil error alone.
func runAuth(flags *rootFlags, opts authOptions) (appPkg.SyncResult, bool, error) {
	if err := flags.requireWritable(); err != nil {
		return appPkg.SyncResult{}, false, err
	}
