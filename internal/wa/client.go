...
// IsGroupJID reports whether jid addresses a group chat.
...
// RevokeMessageForEveryone deletes msgID in chat, either for everyone (a
// revoke, mirroring RevokeMessage) or just locally (DeleteMessageForMe) when
// forEveryone is false. It exists alongside RevokeMessage for callers (delete
// and sync commands) that work with a single bool flag instead of the two
// distinct upstream operations.
func (c *Client) RevokeMessageForEveryone(ctx context.Context, chat types.JID, msgID string, forEveryone bool) error {
	if !forEveryone {
		return c.DeleteMessageForMe(ctx, types.MessageInfo{
			MessageSource: types.MessageSource{Chat: chat},
			ID:            types.MessageID(msgID),
			Timestamp:     time.Now(),
		}, false)
	}
	_, err := c.RevokeMessage(ctx, chat, types.MessageID(msgID))
	return err
}
