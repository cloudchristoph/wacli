...
func (a *App) runSyncFollow(ctx context.Context, maxReconnect time.Duration, messagesStored, connectionEpoch *atomic.Int64, disconnected <-chan struct{}, staleReconnect <-chan staleReconnectRequest, fatal <-chan error) (SyncResult, error) {
	for {
		select {
		case <-ctx.Done():
...
func (a *App) runSyncUntilIdle(ctx context.Context, idleExit, maxReconnect time.Duration, messagesStored, lastEvent *atomic.Int64, disconnected <-chan struct{}, fatal <-chan error) (SyncResult, error) {
