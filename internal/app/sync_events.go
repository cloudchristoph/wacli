...
func (a *App) addSyncEventHandler(ctx context.Context, opts SyncOptions, messagesStored, lastEvent *atomic.Int64, disconnected chan<- struct{}, staleReconnect chan<- staleReconnectRequest, reportFatal func(error), enqueueMedia func(string, string), enqueueWebhook func(wa.ParsedMessage), limits *syncStorageLimits, ps *syncPresence) uint32 {
	...
	return a.wa.AddEventHandler(func(evt interface{}) {
		...
		case *events.LoggedOut:
			a.emitOrPrint("logged_out", map[string]any{"reason": v.Reason.String()},
		...
		case *events.TemporaryBan:
			a.emitOrPrint("temporary_ban", map[string]any{"code": v.Code.String(), "expire": v.Expire.String()},
				"\n%s\n", v.String())
			reportFatal(fmt.Errorf("temporarily banned: %s", v.Code))
		case *events.ClientOutdated:
			a.emitOrPrint("client_outdated", nil,
		...
		case *events.ConnectFailure:
			a.emitOrPrint("connect_failure", map[string]any{"reason": v.Reason.String(), "message": v.Message},
